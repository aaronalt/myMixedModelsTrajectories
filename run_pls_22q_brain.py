...
# Filter to 22q
df_22q = df[df['grouping'] == 1].copy().reset_index(drop=True)
print(f"[4] After 22q filter: {len(df_22q)} observations, {df_22q['subj_id'].nunique()} subjects")
...
covariates = [df_22q['Gender_bin'].values, df_22q['measure_eTIV'].values,
              df_22q['euler_z'].values]

# Claustrum
for hemi in ['clau_lh', 'clau_rh']:
    raw = df_22q[hemi].values.copy()
    resid, _ = compute_residuals(raw.reshape(-1, 1), covariates)
    df_22q[f'{hemi}_resid'] = resid.ravel() + raw.mean()

# Brain parcellations
brain_data = df_22q[brain_cols].values
...
subj_counts = df_22q['subj_id'].value_counts()
...
for subj in multi_subjs:
    sub = df_22q[df_22q['subj_id'] == subj]
    ages = sub['age'].values
...
print("\n" + "=" * 70)
print("BEHAVIORAL PLS: Claustrum slopes vs Brain Region slopes (22q)")
print("=" * 70)
...
# =========================================================================
# Leave-one-out cross-validation of score correlations
# =========================================================================
...
print(f"{'='*70}")
n = X.shape[0]
loo_brain_scores = np.zeros((n, n_lvs))
loo_region_scores = np.zeros((n, n_lvs))
...
for i in range(n):
  ...
  U_loo, S_loo, V_loo, _ = run_svd(X_train, Y_train)

  # Sign-align to full-sample solution
  for lv in range(n_lvs):
      if np.dot(U_loo[:, lv], U[:, lv]) < 0:
          U_loo[:, lv] *= -1
          V_loo[:, lv] *= -1

  # Project held-out subject using training weights
  loo_brain_scores[i] = (X_test @ V_loo).ravel()
  loo_region_scores[i] = (Y_test @ U_loo).ravel()

for lv in range(n_lvs):
  r_loo, p_loo = stats.pearsonr(loo_brain_scores[:, lv], loo_region_scores[:, lv])
  r_insample, _ = stats.pearsonr(brain_scores[:, lv], region_scores[:, lv])
  print(f"\nLV{lv+1}:")
  print(f"  In-sample score correlation:  r = {r_insample:.3f}")
  print(f"  LOO score correlation:        r = {r_loo:.3f}, p = {p_loo:.4f}")
  shrinkage = 1 - r_loo / r_insample if r_insample > 0 else float('nan')
...
results_csv.to_csv(os.path.join(out_dir, 'pls_brain_22q.csv'), index=False)

savemat(os.path.join(out_dir, 'pls_brain_22q.mat'), {
    'U': U, 'S': S, 'V': V,
...
slopes_df.to_csv(os.path.join(out_dir, 'pls_brain_slopes_22q.csv'), index=False)
